...
(function () {
  ...
  P._venaInit = function () {
    ...
    this._vena = {
      ...
      wikiIdx: null, episode: null, forge: {}, storeOffline: false
    };
    this._turnSeq = 0;
    this.setState({ book: '', catalogs: [] });
  ...
  P._venaEvent = function (e) {
    ...
    switch (e.name) {
      ...
      case 'model:progress':
        if (this.state.dl.status === 'downloading') {
      ...
      case 'forge:progress':
        this._vena.forge[p.bookId] = { pct: p.pct | 0, stage: p.stage };
        this.setState({ forgePct: p.pct | 0 });
        return;
      case 'forge:done':
  ...
  P._venaTail = function (L) {
    ...
    const img = D.image;
    const hasPaint = !!(img && img.tier && img.tier !== 'none');
    const paintModels = [{
      id: 'paint',
    ...
    /* gate probes — the real run_probes(bookId, 12) */
    const gateTest = () => {
      ...
      V.call('run_probes', { bookId: meta.id, n: 12 }).then(rs => {
        const n = (rs || []).length;
        const leaks = (rs || []).filter(r => r.leaked).length;
        this.setState({
          gateState: 'idle',
          gateResult: (n - leaks) + '/' + n + ' FUTURE PROBES BLOCKED ✓ · ' + leaks + ' LEAKS'
        });
      }).catch(e => {
    ...
    /* relay config — real set_api_config / test_relay / list_relay_models */
    const provs = [
      ...
      { k: 'custom', label: 'CUSTOM / LOCAL', url: '' }
    ];
    ...
    return {
      ...
      compReady: compReadyR, compFresh: compFreshR, compForging: compForgingR, compEmpty: compEmptyR,
      ...
      provChips: provs.map(p => ({
        ...
        pick: () => this.setState({ relayProv: p.k, relayUrl: p.url || st.relayUrl })
      })),
      ...
      relayTest,
      relayTestLabel: st.relayTestSt === 'busy' ? 'TESTING…' : 'TEST THE RELAY',
