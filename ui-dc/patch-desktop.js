...
(function () {
  ...
  P._venaReaderA11y = function () {
    ...
    // The theme variables (--ink/--panel/…) are scoped to .vhub[data-vtheme];
    // a body-appended overlay resolves NONE of them — invisible in dark. The
    // controls must live INSIDE the themed root, and they sit top-right as a
    // compact Aa toggle that opens the size/spacing panel.
    if (box && box.parentElement !== root) { box.remove(); box = null; }
    if (!box && root) {
      box = document.createElement('div');
      box.setAttribute('data-vena-type', '1');
      box.style.cssText = 'position:absolute;right:18px;top:70px;z-index:80;display:flex;flex-direction:column;align-items:flex-end;gap:6px';
      const tgl = document.createElement('button');
      tgl.textContent = 'Aa';
      tgl.title = 'Text size & line spacing';
      tgl.setAttribute('aria-label', 'Text size and line spacing');
      tgl.setAttribute('aria-expanded', 'false');
      tgl.style.cssText = "background:var(--panel);border:2.5px solid var(--ink);color:var(--ink);min-width:40px;min-height:40px;font-family:'IBM Plex Mono',monospace;font-size:14px;font-weight:700;cursor:pointer;box-shadow:4px 4px 0 var(--shdw)";
      tgl.addEventListener('click', () => {
        this._typeOpen = !this._typeOpen;
        tgl.setAttribute('aria-expanded', this._typeOpen ? 'true' : 'false');
        this._venaReaderA11y();
      });
      box.appendChild(tgl);
      const panel = document.createElement('div');
      panel.setAttribute('role', 'group');
      panel.setAttribute('aria-label', 'Text size and line spacing controls');
      panel.style.cssText = 'display:none;gap:6px;background:var(--panel);border:2.5px solid var(--ink);box-shadow:4px 4px 0 var(--shdw);padding:6px';
    ...
    if (box.__vPanel) box.__vPanel.style.display = this._typeOpen ? 'flex' : 'none';
    // the canonical reader toolbar ships its own (decorative) Aa button —
    // wire it to the same panel so it isn't a dead control
    Array.prototype.forEach.call(root.querySelectorAll('button,div'), el => {
      if (el.__vWired || box.contains(el) || (el.textContent || '').trim() !== 'Aa') return;
      if (el.querySelector && el.querySelector('*') && el.tagName !== 'BUTTON') return;
      el.__vWired = true;
