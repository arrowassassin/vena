...
(function () {
  ...
  P._applyReaderA11y = function () {
    ...
    var root = document.querySelector(".vhub");
    ...
    // Theme vars are scoped to .vhub[data-vtheme] — a body-appended overlay
    // resolves none of them (invisible in dark). Live INSIDE the themed root,
    // as a compact Aa toggle at the top right that opens the controls.
    if (box && box.parentElement !== root) {
    ...
    if (!box && root) {
      ...
      box.style.cssText = "position:absolute;right:10px;top:64px;z-index:80;display:flex;flex-direction:column;align-items:flex-end;gap:5px";
      var tgl = document.createElement("button");
      tgl.textContent = "Aa";
      tgl.setAttribute("aria-label", "Text size and line spacing");
      tgl.setAttribute("aria-expanded", "false");
      tgl.style.cssText = "background:var(--panel);border:2.5px solid var(--ink);color:var(--ink);min-width:42px;min-height:42px;font-family:'IBM Plex Mono',monospace;font-size:14px;font-weight:700;cursor:pointer;box-shadow:3px 3px 0 var(--shdw)";
      tgl.addEventListener("click", function () {
        self._typeOpen = !self._typeOpen;
        tgl.setAttribute("aria-expanded", self._typeOpen ? "true" : "false");
        self._applyReaderA11y();
      });
      box.appendChild(tgl);
      var panel = document.createElement("div");
      panel.setAttribute("role", "group");
      panel.setAttribute("aria-label", "Text size and line spacing controls");
      panel.style.cssText = "display:none;gap:5px;background:var(--panel);border:2.5px solid var(--ink);box-shadow:3px 3px 0 var(--shdw);padding:5px";
    ...
    if (box.__vPanel) box.__vPanel.style.display = self._typeOpen ? "flex" : "none";
