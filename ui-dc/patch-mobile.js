...
(function () {
  ...
  P._loadManga = function () {
    ...
    var meta = metas.filter(function (m) { return m.profile === "comic" && m.slug === self.state.book; })[0]
      || metas.filter(function (m) { return m.profile === "comic"; })[0];
    ...
    V.call("get_manga_pages", { bookId: meta.id }).then(function (r) {
      ...
      self._manga = count > 0
        ? { bookId: meta.id, title: String(meta.title || "").toUpperCase(), count: count, cache: {}, pending: {} }
        : null;
      self.setState(self._manga ? { mangaPage: 1 } : {});
    }).catch(function (e) { self._manga = null; self._up(e); });
  };
    ...
    if (this._paintTiers && this._paintTiers.length) {
      ...
      v.paintModels = this._paintTiers.map(function (t) {
        ...
        return {
          ...
          btnLabel: installed ? "INSTALLED ✓" : downloading ? "GETTING…" : "DOWNLOAD",
