//! The Vena Tauri 2 shell — commands + `Builder`, gated behind the `tauri`
//! feature. It lives in the LIB (not the bin) because mobile targets load the
//! app as `libvena_app.so` and call `run()` via `tauri::mobile_entry_point`; the
//! desktop `vena` bin is a thin `fn main() { vena_app::run() }`. Every command is
//! a thin wrapper over `crate::AppApi` (where the real logic + tests live).
//! Events emitted: forge:progress, forge:done, companion:stage, companion:token,
//! companion:done, model:progress, store:progress, image:progress, image:done.

use crate::api::{AiStatus, AppApi, ImageStatus, RelayTest, StoreItem};
use std::sync::Arc;
...
impl crate::KeyStore for KeychainStore {
...
#[tauri::command]
fn delete_paint_model(api: State<'_, Api>, tier: String) -> Result<serde_json::Value, VenaError> {
    api.delete_paint_model(&tier)
}
...
/// The Tauri shell entry point. On desktop the `vena` bin calls this from `main`;
/// on mobile the `tauri::mobile_entry_point` macro exports it as the symbol the
